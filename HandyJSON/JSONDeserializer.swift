...
extension Property {
    ...
    internal static func _transform(rawData dict: NSDictionary, toPointer pointer: UnsafePointer<Byte>, toOffset currentOffset: Int, byMirror mirror: Mirror, withMapper mapper: HelpingMapper) -> Int {
        ...
        mirror.children.forEach({ (child) in
            ...
            guard let value = self.valueForKeypath(key,inDict:dict) as? NSObject else {
                mutablePointer = mutablePointer.advancedBy(size)
                currentOffset += size
                return
            }
    ...
    static func valueForKeypath(keypath:NSString,inDict:AnyObject) ->NSObject{
        guard let dict=inDict as? NSDictionary else {
            return ""
        }
        if let value = inDict[keypath] as? NSObject{
            return value
        }
        let range = keypath.rangeOfString(".")
        if range.location == NSNotFound {
            return ""
        }
        let currentKey = keypath.substringToIndex(range.location)
        let newKeyPath = keypath.substringFromIndex(range.location+1)
        return self.valueForKeypath(newKeyPath,inDict: dict[currentKey]!)
    }
